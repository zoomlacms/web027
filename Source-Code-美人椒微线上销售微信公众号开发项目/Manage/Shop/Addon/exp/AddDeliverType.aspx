       ...
       <%--         <div style="border-bottom:1px solid #ddd;padding-bottom:5px;">
                ...
                <div ng-repeat="item in list track by $index" class="margin_t5">
                 ...
                 <%--   <input type="text" ng-model="item.name" class="text_xs" placeholder="标识"/>--%>
                    <label style="min-width:45px;"><input type="checkbox" ng-model="item.enabled" />{{item.alias}}</label>
...
<script type="text/javascript">
    ...
    angular.module("app", []).controller("appCtrl", function ($scope) {
        scope = $scope;
        $scope.newModel = function (name, alias) {
            //快递名,别名,计价方式,初始价,续价,包邮方式,多少件包邮,多少钱包邮
            return { "name": name, "alias": alias, mode: 1, enabled: true, price: 10, plus: 0, free_sel: 0, free_num: 0, free_money: 0 };
        }
        $scope.add = function () {
        ...
        $scope.save = function () {
            for (var i = 0; i < $scope.list.length; i++) {
                ...
                if (ZL_Regex.isEmpty(model.name)) { alert("快递标识不能为空"); return false; }
                if (ZL_Regex.isEmpty(model.alias)) { alert("快递别名不能为空"); return false; }
            }
        ...
        if (ZL_Regex.isEmpty($("#Fare_Hid").val())) {
            $scope.list.push($scope.newModel("exp", "快递"));
            $scope.list.push($scope.newModel("ems", "EMS"));
            $scope.list.push($scope.newModel("mail", "平邮"));
        }
        else {

<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="mbview.aspx.cs" Inherits="Design_h5_mbview" EnableViewState="false" ClientIDMode="Static" %><!DOCTYPE html>
<html>
<head>
...
<script>
    ...
    if(window.wx)
    {
        ...
        <%="var desc =\""+(string.IsNullOrEmpty(pageMod.Meta) ? pageMod.Title : pageMod.Meta).Replace("\r","").Replace("\n","")+"\";" %>
        ...
        wx.ready(function() {
            ...
            if(typeof(conf.automusic)=="undefined"||conf.automusic){
                $("#music_btn").click();page.music.$audio[0].play();
            }
           
        });
    }
    else{
        playMusic(pageData.scence_conf);
    }
    function playMusic(conf)
    {
        if(typeof(conf)=="string"){conf=JSON.parse(conf);}
        if(typeof(conf.automusic)=="undefined"||conf.automusic){
          
            setTimeout(function(){$("#music_btn").click();page.music.$audio[0].play();console.log(page.music.$audio[0]);},2000);
        }
    }
</script>
</body>
